  ...
  class RenderMainColumnTag < Liquid::Tag
    ...
    def render(context)
      ...
      if @text[0].start_with?('http://', 'https://','//')
        "<figure><img src='#{@text[0]}' alt='#{@text[1]}'/><figcaption class='maincolumn-figure'>#{label}</figcaption></figure>"
      else
        "<figure><img src='#{baseurl}/#{@text[0]}' alt='#{@text[1]}'/><figcaption class='maincolumn-figure'>#{label}</figcaption></figure>"
      end
    end
  end
end
